    ...
    <div class="container">
      ...
      <div class="homework-list-card">
        ...
        <div class="search-section">
          <el-input v-model="searchForm.title" placeholder="作业标题" clearable style="width: 200px" @keyup.enter="handleSearch" />
          <el-select v-model="searchForm.status" placeholder="作业状态" clearable style="width: 150px">
        ...
        <el-table :data="homeworkList"  stripe style="width: 100%">
          ...
          <el-table-column prop="dateRange" label="作业日期" width="380" />
          <el-table-column prop="status" label="作业状态" width="120" align="center">
          ...
          <el-table-column label="操作" width="180" align="center">
            <template #default="{ row }">
              <el-button type="primary" size="small" @click="viewHomework(row)">查看</el-button>
              <el-button type="warning" size="small" @click="editHomework(row)" :disabled="row.status === '已提交'">修改</el-button>
            </template>
          </el-table-column>
        </el-table>
...
const fetchHomeworkList = async () => {
  try {
    const res = await getMyHomework(route.params.id);
    const data = res.data;
    if (data) {
      homeworkList.value = data.homeworkList || data.records || data || [];
...
const viewHomework = row => {
  router.push(`/profile/homework/${route.params.id}/answer/${row.id}`);
};
