...
// 复用 uploadFile 作为视频上传（后端同一个接口）
const uploadVideo = (formData: FormData) => uploadFile(formData);
...
props.editorConfig.MENU_CONF!["uploadImage"] = {
  async customUpload(file: File, insertFn: InsertFnTypeImg) {
    ...
    try {
      ...
      const fullUrl = data.fileUrl.startsWith("http") ? data.fileUrl : `${baseUrl}${data.fileUrl}`;
      insertFn(fullUrl);
    } catch (error) {
      console.log(error);
...
props.editorConfig.MENU_CONF!["uploadVideo"] = {
  async customUpload(file: File, insertFn: InsertFnTypeVideo) {
    ...
    try {
      ...
      console.log(error);
      ElMessage.error("视频上传失败，请检查网络或文件大小");
    }
  }
};
