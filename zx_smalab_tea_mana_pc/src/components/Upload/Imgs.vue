...
const beforeUpload: UploadProps["beforeUpload"] = rawFile => {
  ...
  if (!imgType)
    ElNotification({
      ...
      type: "warning",
      customClass: 'ElNotification_style'
    });
  if (!imgSize)
    setTimeout(() => {
      ElNotification({
...
const uploadSuccess = (response: { url: string, minioName: string } | undefined, uploadFile: UploadFile) => {
  if (!response) return;
  uploadFile.url = response.url;
  uploadFile.name = response.minioName;
